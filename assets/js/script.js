...
yandex.addEventListener('keydown', yandexCardNumberValidate);
checkVisa.addEventListener('keydown', checkVisaAdvance)
...
function yandexCardNumberValidate(e) {
  let value = this.value.replace(/\s+/g, '');
  let isBackspace = e.key === 'Backspace';
  if ((e.key.length === 1 && /^[^\d\s]+$/.test(e.key)) || (!isBackspace && value.length === 16) ) {
...
function checkVisaAdvance(e) {
  let value = this.value.replace(/\s+/g, '');
  let isBackspace = e.key === 'Backspace';
  if ((e.key.length === 1 && /^[^\d\s]+$/.test(e.key)) || (!isBackspace && value.length === 10) ) {
    e.preventDefault();
    return false;
  } else if (!isBackspace && value.length < 2) {
    checkVisa.style.border = '1px solid green'
  }

  this.value = value.split('').reverse().join('').replace(/\B(?=(\d{3})+(?!\d))/g, " ").split('').reverse().join('').trim();
};